...
exports.GetSecondaryStockEntry=(req,res)=>{
    ...
    const{fromDate,billId,toDate}=req.body
        ...
        if(billId  && !fromDate && !toDate){
            ...
            query=`SELECT *
                   FROM stock
                   WHERE bill_date BETWEEN "${fromDate}" AND "${toDate}" ORDER BY bill_date DESC,bill_id ASC `
        }
  
        else if(billId && fromDate && toDate){
            query=`SELECT *
                   FROM stock
                   WHERE bill_date BETWEEN "${fromDate}" AND "${toDate}" AND  bill_id REGEXP "${billId}" ORDER BY bill_date DESC,bill_id ASC `
        }
